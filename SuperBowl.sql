...
SELECT COUNT(*) as num_games,
    CASE 
        WHEN Winner_Pts - Loser_Pts > 30 THEN "big blow out"
        WHEN Winner_Pts - Loser_Pts > 20 THEN "blow out"
        ELSE "Close Game"
    END as "game_closeness"
FROM superbowls
GROUP BY game_closeness;

/*most points*/
SELECT max(Winner_Pts) as max_points FROM superbowls; 

/*least points*/
SELECT min(Loser_Pts) as min_points FROM superbowls;

/*Winners that average over 10 points*/
SELECT Winner, AVG(Winner_Pts) as avg_points FROM superbowls GROUP BY Winner HAVING avg_points>10;

/*Winners in superbowl*/
SELECT Winner As winning_teams FROM superbowls GROUP BY winning_teams;

/*Losers in superbowl*/
SELECT Loser As losing_teams FROM superbowls GROUP BY losing_teams;

/*High scoring games*/
SELECT COUNT(*) AS high_score_games FROM superbowls WHERE Loser_Pts >20 AND Winner_Pts >30;